...
public class VideoSystemXmlConfigTest {
	...
	@Autowired
	Avengers dvd1; // 안좋은 방식이다 인터페이스로 주입을 받아라
	...
	@Autowired
	@Qualifier("avengersExpantionPack")
	DigitalVideoDisc dvd8;

	@Autowired
	@Qualifier("avengersExpantionPack2")
	...
	public void testDVD8Null() {
		...
		assertEquals(
				"BlankDisc [title=Avengers Expantion Pack, studio=MARVEL, actors=[Robert Downey Jr., Scarlett Johansson, Chris Evans]]",
				systemOutRule.getLog().replace("\r\n", "").replace("\n", ""));
	}
	...
	public void testDVD9Null() {
		...
		assertEquals(
				"BlankDisc [title=Avengers Expantion Pack2, studio=MARVEL, actors=[Robert Downey Jr., Scarlett Johansson, Chris Evans]]",
				systemOutRule.getLog().replace("\r\n", "").replace("\n", ""));
	}
